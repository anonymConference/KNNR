...
Nsample_list = [100,250,500,750,1000]#25000]##[100]#[2500,5000,7500,10000, 25000]#[100]#
eps = 1.0
n_reps = 30
...
for Nsample in Nsample_list:
    ...
    for i_rep in range(n_reps):
        ...
        #initial_state_bool_df = initial_state_list[end_state_np!=1,:]
        match_df = pre_state_np[end_state_np!=1,:]
        next_state_df = post_state_np[end_state_np!=1,:]
        reward_df = reward_np[end_state_np!=1] 
        
        #inital_states = pre_state_np[initial_state_bool_df==1]
        used_np = np.zeros(match_df.shape[0])
        ...
        #n_neighbors= int(Nsample**0.25)#5
        # #
        # #
        # #
        # st = time.time()
        
        #nbrs = NearestNeighbors(n_neighbors=n_neighbors+env_config['time_horizon'],  algorithm='ball_tree',n_jobs=72).fit(match_df )
        ...
        start_vec = np.array(range(Nsample-1))*30
        start_ind = np.random.choice(start_vec,size = num_resample,replace = False)
        
        used_np[start_ind] = 1
        
        initial_states = match_df[used_np==1][:,:-1]
        match_df = np.delete(match_df, used_np == 1,axis=0)
        next_state_df = np.delete(next_state_df, used_np == 1,axis=0)
        reward_df = np.delete(reward_df, used_np == 1,axis=0)
        ...
        for i in range(num_resample):
            ...
            state_wo_mask = initial_states[i]
            ...
            for t_res in range(time_horizon):
                ...
                state_match =  np.append(state_wo_mask,action).astype('float64')
                #state_pre = state_wo_mask[:-1]
                distances = euclidean_distances(state_match.reshape(1, -1) ,match_df)
                
                NN_ind = np.argmin(distances)
                state_wo_mask = next_state_df[NN_ind,:]
                
                
                rt = reward_df[NN_ind]
                
                match_df = np.delete(match_df, NN_ind,axis=0)
                next_state_df = np.delete(next_state_df, NN_ind,axis=0)
                reward_df = np.delete(reward_df, NN_ind,axis=0)
                

                #state_post = state_wo_mask[:-1]
                #print(reward_df[indices[0,rand_NN]])
                #reward_adj = 0#- get_waste_diff(state_pre, state_post, waste_helper)
                total_reward += rt #reward_df[masked_indices[0,rand_NN]]#+reward_adj 
                #print('Eval Number ' + str(random_seed)+ ' is at t='+ str(t_res) +'with total reward=' + str(total_reward))
                action_mask = get_action_mask(state_wo_mask, env_config["bag_capacity"])
                state = {'action_mask':action_mask, 'real_obs': state_wo_mask} 
