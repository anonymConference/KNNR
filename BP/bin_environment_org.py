# -*- coding: utf-8 -*-
"""
Created on Mon Dec 12 11:57:14 2022
...
class BinPackingGymEnvironment(gym.Env):
    ...
    def __init__(self, env_config={}):
        ...
        self.max_item = np.max(self.item_sizes)
    ...
    def calc_lambda(self):
        if np.sum(self.num_bins_levels[self.num_bins_levels!=0])==0:
            lambda_poi = self.max_item
        else:
            #lambda_poi = np.mean((20-np.where(num_bins_levels!=0)[0]))# Not volume weighted
            weights =  self.num_bins_levels[self.num_bins_levels!=0]/np.sum(self.num_bins_levels[self.num_bins_levels!=0])
            
            if weights ==1:
                lambda_poi = self.num_bins_levels[self.num_bins_levels!=0]
            else:
                lambda_poi = np.average((self.bag_capacity-np.where(self.num_bins_levels!=0)[0]),weights=weights )
        lambda_poi = np.min([lambda_poi,self.max_item]) 
        return lambda_poi

    def get_truncated_poi(self,size=1):
        
        cutoff_upper = poisson.cdf(self.max_item,self.lambda_poi) 
        cutoff_lower = poisson.cdf(0,self.lambda_poi) 
        u = uniform.rvs(scale = (cutoff_upper-cutoff_lower), loc = cutoff_lower, size =size)
    
        truncated_poi = poisson.ppf(u,self.lambda_poi)
        return truncated_poi


    def __get_item(self):
        self.lambda_poi = self.calc_lambda()
        truncated_poi = self.get_truncated_poi()[0]
        #num_items = len(self.item_sizes)
        item_index = int(truncated_poi)-1#np.random.choice(num_items, p=self.item_probabilities)
