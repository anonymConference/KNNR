...
Nsample_list = [7500]#[100,250,500,750,1000]#[100]#, 25000]#[100]#
eps = 1.0
n_reps = 30
...
for Nsample in Nsample_list:
    ...
    for i_rep in range(n_reps):
        ...
        features = np.concatenate((match_df , action_np.T), axis=1).astype('float64') 
        
        action_modified_np = np.apply_along_axis(get_action_modified, 1, next_state_df)
        
        features_modified = np.concatenate((next_state_df,action_modified_np[:, np.newaxis] ), axis=1)
        
            
            
        Q_target = np.zeros(reward_df.size)
        
        n_neighbors= int(Nsample**0.5)
        ...
        init_features = np.concatenate((inital_states,action_modified_np[:, np.newaxis]), axis=1)
