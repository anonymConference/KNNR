...
Nsample_list =[25000]# [100,250,500,750,1000]#[100]#[2500,5000,7500,10000]#[25000]#[100]#[10000]#[100,250,500,750,1000]#, ,50000,100000] # Number of sample trajectories
...
for Nsample in Nsample_list:
    ...
    for i_rep in range(n_reps):
        ...
        t_np = np.array(df['time'])
        
        resampled_reward_estimate = np.dot(reward_df,IS_np)/Nsample
        ...
        features = poly.fit_transform(np.array(df[match_columns]))

        features_state = np.array(df[['next_state1','next_state2']])
        actions = np.matmul(features_state, -K.T)
        features_modified = poly.fit_transform(np.concatenate((features_state,actions), axis=1))
        
        #reg_old = 0
        
        WDR_np = np.zeros([df['reward'].size,T])
        ...
        init_features = poly.fit_transform(np.concatenate((init_state ,actions), axis=1))
        V = reg.predict(init_features)
        reward_stored[i_rep, N_ind] = np.mean(V)+reward_dummy
