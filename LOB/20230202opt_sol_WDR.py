...
N_traj = 1000000
Nsample_list = [25000]#[25000]#[100,250,500,750,1000]#[2500,5000]#[25000]##[100]#[25000]#[10000]##[50000]#,,100,250,500,750,1000,#[100,250,500,750,1000]#
resample_ratio = 5
n_reps = 30
...
if __name__ == '__main__':
    ...
    for Nsample in Nsample_list:
        ...
        for i_rep in range(n_reps):
            ...
            #Q_last = -next_state_df[:,1]*(next_state_df[:,0]-alpha*next_state_df[:,1])
            WDR_np = np.zeros([reward_df.size,T])
            ...
            for t in range(T-1):
                WDR_t = WDR_np[:,t] 
                #print(np.sum(IS_df[t_df==t]))
                reward_dummy = reward_dummy + np.dot(WDR_t[t_df==t],IS_df[t_df==t])/np.sum(IS_df[t_df==t]) 
                #print( np.dot(rewards[t_np==t],IS[t_np==t])/IS_total_weights[t])
                 
            # print(reward_dummy)
            reward_stored[i_rep, N_ind] = np.mean( V ) + reward_dummy
            print(reward_stored[i_rep, N_ind])
            #np.dot(rewards,IS)/sum(IS)#
        N_ind=N_ind+1